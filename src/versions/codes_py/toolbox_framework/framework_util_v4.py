        ...
        if dataset is None:
            ...
            if k not in existingKeys:
                ...
                if type(batch_vis[k]) is np.ndarray:
                    ...
                    raise NotImplementedError('Unknown type(batch_vis[k]): k: %s, type: %s' %
                                              (k, type(batch_vis[k])))
        else:
...
def bsv02bsv(bsv0):
    bsv = {}
    for k in bsv0.keys():
        if type(bsv0[k]) is np.ndarray or type(bsv0[k]) is torch.Tensor or \
                type(bsv0[k]) is np.int64 or type(bsv0[k]) is np.int32 or \
                type(bsv0[k]) is np.float64 or type(bsv0[k]) is np.float32:
            bsv[k] = bsv0[k][None]
            ...
            raise NotImplementedError('Unknown type(bsv0[k]): k: %s, type: %s' % (k, type(bsv0[k])))
    return bsv
...
def bsv2bsv0(bsv, visIndex):
    ...
    for k in bsv.keys():
        if type(bsv[k]) is np.ndarray or type(bsv[k]) is torch.Tensor:
            ...
            raise NotImplementedError('Unknown type(bsv[k]): k: %s, type: %s' % (k, type(bsv[k])))
    return bsv0
