...
class RasterizeFunction(Function):
    ...
    def forward_face_index_map(ctx, faces, face_index_map, weight_map,
        ...
        ''' zhuzhu vis check. This is the wrong version.
        from scipy.io import savemat
        a = {}
        a['faces'] = faces
        a['face_index_map'] = face_index_map
        a['weight_map'] = weight_map
        a['depth_map'] = depth_map
        a['face_inv_map'] = face_inv_map
        a['faces_inv'] = faces_inv
        print(ctx.image_size)
        print(ctx.near)
        print(ctx.far)
        print(ctx.return_rgb)
        print(ctx.return_alpha)
        print(ctx.return_depth)
        for k in a.keys():
            a[k] = a[k].detach().cpu().numpy()[0]
        a['face_index_map_new'], a['weight_map_new'], a['depth_map_new'], a['face_inv_map_new'] = \
            rasterize_cuda.forward_face_index_map(faces, face_index_map, weight_map,
        ...
        ''' zhuzhu vis check. This is the right version.
        from scipy.io import loadmat
        a = loadmat('a.mat')
        b = {}
        for k in a.keys():
            if not (k.endswith('_new') or k.startswith('__')):
                print(k)
                b[k] = torch.from_numpy(a[k][None]).cuda()

        face_index_map_new, weight_map_new, depth_map_new, face_inv_map_new = \
            rasterize_cuda.forward_face_index_map(b['faces'], b['face_index_map'], b['weight_map'],
                                                  b['depth_map'], b['face_inv_map'], b['faces_inv'],
                                                  512, 0.1, 100, True, False, False)
        import ipdb
        ipdb.set_trace()
        print(1+1)
        '''
