...
class Decoder(nn.Module):
    ...
    def forwardGradTrack(self, tt4):
        if_def_layer = self.meta['if_def_layer']

        out4 = {}
        if if_def_layer['y0']:
            y0 = self.stage0(tt4['ta'])
            out4['y0'] = y0
        if if_def_layer['y1']:
            x = y0
            x = x[:, :, None, None, None]
        ...
        if if_def_layer['y3']:
            y2 = torch.cat([
                y2[:, :, None, :, :, :],
                torch.zeros(
                    y2.shape[0], y2.shape[1], 3, y2.shape[2], y2.shape[3], y2.shape[4],
                    dtype=y2.dtype, device=y2.device
                )
            ], 2)
            x = torch.cat([y2, tt4['t5']], 1)
            r1 = reluGradTrack(x)
            b1 = bnGradTrack(self.stage3.b1, r1)
            c1 = conv3dGradTrack(self.stage3.c1, b1)
            r2 = reluGradTrack(c1)
            b2 = bnGradTrack(self.stage3.b2, r2)
            t1 = convtranspose3dGradTrack(self.stage3.t1, b2)
            y3 = t1
            out4['y3'] = y3
        if if_def_layer['y4']:
            x = torch.cat([y3, tt4['t4']], 1)
            r1 = reluGradTrack(x)
            b1 = bnGradTrack(self.stage4.b1, r1)
            c1 = conv3dGradTrack(self.stage4.c1, b1)
            r2 = reluGradTrack(c1)
            b2 = bnGradTrack(self.stage4.b2, r2)
            t1 = convtranspose3dGradTrack(self.stage4.t1, b2)
            y4 = t1
            out4['y4'] = y4
        if if_def_layer['y5']:
            x = torch.cat([y4, tt4['t3']], 1)
            r1 = reluGradTrack(x)
            b1 = bnGradTrack(self.stage5.b1, r1)
            c1 = conv3dGradTrack(self.stage5.c1, b1)
            r2 = reluGradTrack(c1)
            b2 = bnGradTrack(self.stage5.b2, r2)
            t1 = convtranspose3dGradTrack(self.stage5.t1, b2)
            y5 = t1
            out4['y5'] = y5
        if if_def_layer['y6']:
            x = torch.cat([y5, tt4['t2']], 1)
            r1 = reluGradTrack(x)
            b1 = bnGradTrack(self.stage6.b1, r1)
            c1 = conv3dGradTrack(self.stage6.c1, b1)
            r2 = reluGradTrack(c1)
            b2 = bnGradTrack(self.stage6.b2, r2)
            t1 = convtranspose3dGradTrack(self.stage6.t1, b2)
            y6 = t1
            out4['y6'] = y6
