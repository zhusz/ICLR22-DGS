        ...
        class DTrainer(Trainer):
            ...
            def forwardNet(cls, batch_thgpu, **kwargs):
                ...
                for samplingMethod in ['nearsurfacemass', 'nearsurfaceair', 'nonsurface']:
                    if 'final%sPCxyzCam' % bt(samplingMethod) not in batch_thgpu.keys():
                        continue
                    ...
                    if samplingMethod in ['nearsurfacemass', 'nearsurfaceair']:
                        ...
                        batch_thgpu['final%sPCpredBisem' % bt(samplingMethod)] = z6
                    elif samplingMethod in ['nonsurface']:
                        z6 = dgs3dLayer(y6, grid)[:, 0, :, :].permute(0, 2, 1)
                        batch_thgpu['final%sPCpredBisem' % bt(samplingMethod)] = z6[:, :, 0]
                        batch_thgpu['final%sPCpredGradBisem' % bt(samplingMethod)] = z6[:, :, 1:]
                    else:
