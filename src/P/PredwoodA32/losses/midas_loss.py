...
def masked_shift_and_scale(depth_preds, depth_gt, mask_valid):
    ...
    tmp = depth_gt_nan.view(depth_gt_nan.size()[:2] + (-1,))
    tmp1 = torch.all(torch.isnan(tmp), dim=2)
    tmp[tmp1[:, :, None].repeat(1, 1, tmp.shape[2])] = 0  # in case empty row
    t_gt = torch.median(tmp[torch.isfinite(tmp)], dim=-1, keepdims=True)[0].unsqueeze(-1)
    ...
    tmp = depth_preds_nan.view(depth_preds_nan.size()[:2] + (-1,))
    tmp1 = torch.all(torch.isnan(tmp), dim=2)
    tmp[tmp1[:, :, None].repeat(1, 1, tmp.shape[2])] = 0  # in case empty row
    t_pred = torch.median(tmp[torch.isfinite(tmp)], dim=-1, keepdims=True)[0].unsqueeze(-1)
