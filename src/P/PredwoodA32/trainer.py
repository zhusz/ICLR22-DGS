...
# PredwoodA32/33
# The same functionality with PredwoodA30, just that rename the configurations, and use A32 to copy into A33 (odd numbers refer to scannet training)
...
from codes_py.toolbox_3D.self_sampling_v1 import mesh_weighted_sampling_given_normal_np, \
    mesh_weighted_sampling_given_normal
...
# PDSR
from .dataset import PHmRenderOmDataset, POmnidataBerkeleyDataset
...
# benchmarking
from Bpredwood2.testDataEntry.hmRenderOmDataset import HmMeshCache, collate_fn_hmRenderOm
...
nanmean = lambda x: x[torch.isfinite(x)].mean()
...
class Trainer(object):
    ...
    def metaDataLoading(self, **kwargs):
        ...
        for datasetConf in datasetConfDict.values():
            if datasetConf['class'] == 'HmRenderOmDataset':
                datasetObj = PHmRenderOmDataset(
                    datasetConf,
                    datasetSplit=datasetConf['trainingSplit'],
                    projRoot=projRoot,
                )
                batchSizePerProcess = datasetConf['batchSizePerProcess']
                num_workers = int(math.ceil(batchSizePerProcess / 4.))  # Having more parallel loaders now turns out to be essential!!
                dataLoader = torch.utils.data.DataLoader(
                    datasetObj, batch_size=batchSizePerProcess, num_workers=num_workers,
                    shuffle=True, drop_last=True, collate_fn=collate_fn_hmRenderOm,
                )
    ...
    def setupMetaVariables(self):
        ...
        meta = self._meta_initialize_hm_house_vert_world_cache(meta)
        self.meta = meta

    @staticmethod
    def _meta_initialize_hm_house_vert_world_cache(meta):
        meta['hmMeshCache'] = HmMeshCache()
        return meta
    ...
    def setupMonitor(self):
        ...
        self.testDataEntry = getTestDataEntryDict(
            wishedTestDataNickName=['hmFsOfficialValSplit10'])['hmFsOfficialValSplit10']
    ...
    def stepMonitorImmediateFlow(self, batch_vis, **kwargs):  # use the current training data for immediate use (e.g. with augmentation)
        ...
        ben = {}
        batch_vis_now = batchExtractParticularDataset(batch_vis, 'hmRenderOm')
        for visIndex in range(min(4, batch_vis_now['index'].shape[0])):
            print('    Stepping Visualizer ImmediateFlow: %d' % visIndex)
            assert testDataEntry['datasetObj'].dataset == 'hmRenderOm'
            ...
            bsv0 = benchmarkingGeometry3DHmFunc(
                bsv0_initial,
                hmMeshCache=self.meta['hmMeshCache'],
    ...
    def stepMonitorVal(self, **kwargs):
        ...
        for j in range(j1, j2):
            ...
            # step
            if testDataNickName in ['hmFsOfficialValSplit10']:
                bsv0 = benchmarkingGeometry3DHmFunc(
                    bsv0_initial,
                    hmMeshCache=self.meta['hmMeshCache'],
                    datasetObj=datasetObj,
    ...
    @classmethod
    def samplingBatchForHmTHGPU(cls, batch_thcpu, batch_thgpu, **kwargs):
        ...
        for j in range(B):
            ...
            # cache loading
            tmp0 = meta['hmMeshCache'].call_cache_hm_house_vert_world_0(
                houseID0=houseID0,
            ...
            faceArea0 = tmp0['faceArea0']

            nFace = int(face0.shape[0])
            faceFlag0 = batch_thcpu['faceFlagUntrimmed_hmRenderOm'][j][:nFace].numpy().astype(bool)
            face0 = face0[faceFlag0]
            faceNormalWorld0 = faceNormalWorld0[faceFlag0]
            faceArea0 = faceArea0[faceFlag0]
            faceVertWorld0 = vertInfo2faceVertInfoNP(vertWorld0[None], face0[None])[0]
            cudaDevice = batch_thgpu['index' + _dataset].device
            tmp1 = mesh_weighted_sampling_given_normal(
                datasetConf['numSamplingFinalFullsurface'],
                torch.from_numpy(np.cumsum(faceArea0)).to(cudaDevice),
                torch.from_numpy(faceVertWorld0).to(cudaDevice),
                torch.from_numpy(faceNormalWorld0).to(cudaDevice),
                {}, {},
                # ifFaceNormal0alreadyNormalized=True,
                # cudaDevice=batch_thgpu['index' + _dataset].device,
            )
            finalFullsurfacePCxyzWorld[j, :, :] = tmp1[0].detach().cpu().numpy()
            finalFullsurfacePCnormalWorld[j, :, :] = tmp1[1].detach().cpu().numpy()
        ...
        del houseID0, tmp0, vertWorld0, face0, faceNormalWorld0, faceArea0, nFace
        del faceFlag0, tmp1
    ...
    def batchPreprocessingTHGPU(self, batch_thcpu, batch_thgpu, **kwargs):
        ...
        batch_thgpu = self.samplingBatchForHmTHGPU(
            batch_thcpu,
            batch_thgpu,
            datasetObj=self.datasetObjDict['hmRenderOm'],
            cudaDevice=self.cudaDeviceForAll,
    ...
    @classmethod
    def forwardNetForHm(cls, batch_thgpu, **kwargs):
    ...
    def forwardLoss(cls, batch_thgpu, **kwargs):
        ...
        if 'lossDepthRegL1' in wl.keys() and wl['lossDepthRegL1'] > 0:
            depthRaw = batch_thgpu['depthForUse_hmRenderOm']
            depthPred = batch_thgpu['depthPred_hmRenderOm']
        ...
        if 'lossNearsurfaceClf' in wl.keys() and wl['lossNearsurfaceClf'] > 0:
            tmp = F.binary_cross_entropy(
                batch_thgpu['finalNearsurfacePCpredOccfloat_hmRenderOm'],
                batch_thgpu['finalNearsurfacePCoccfloat_hmRenderOm'],
                reduction='none',
            )
            batch_thgpu['lossNearsurfaceClf'] = (
                    tmp * (batch_thgpu['finalNearsurfacePCmaskfloat_hmRenderOm'] > 0).float()
            ).mean()
            del tmp
        ...
        if ifRequiresGrad and 'lossSelfnonsurfaceGradOccfloat' in wl.keys() \
            ...
            batch_thgpu['lossSelfnonsurfaceGradOccfloat'] = torch.abs(
                batch_thgpu['finalSelfnonsurfacePCpredGradOccfloat_hmRenderOm'] *
                (batch_thgpu['finalSelfnonsurfacePCmaskfloat_hmRenderOm'] > 0).float()[:, :, None]
            ).mean()

        # lossUnarysurfaceNormal
        if ifRequiresGrad and 'lossUnarysurfaceNormal' in wl.keys() and wl['lossUnarysurfaceNormal'] > 0:
            mask = (batch_thgpu['finalUnarysurfacePCmaskfloat_hmRenderOm'] > 0)  # (B, Q)
            predGrad = batch_thgpu['finalUnarysurfacePCpredGradOccfloat_hmRenderOm']  # (B, Q, 3)
            labelGrad = batch_thgpu['finalUnarysurfacePCnormalCam_hmRenderOm']  # (B, Q, 3)
        ...
        for edgeOrPlanar in ['edge', 'planar']:
            if ifRequiresGrad and ('lossLocal%sPairwiseSurfaceNormal' % bt(edgeOrPlanar) in wl.keys()) and \
                ...
                maskA = (batch_thgpu['finalLocal%sPairwiseSurfaceAPCmaskfloat_hmRenderOm' % bt(edgeOrPlanar)] > 0)
                maskB = (batch_thgpu['finalLocal%sPairwiseSurfaceBPCmaskfloat_hmRenderOm' % bt(edgeOrPlanar)] > 0)
                mask = maskA & maskB  # (B, Q)
                predAGrad = batch_thgpu['finalLocal%sPairwiseSurfaceAPCpredGradOccfloat_hmRenderOm' % bt(edgeOrPlanar)]
                predANorm = torch.clip(torch.norm(predAGrad, dim=2, p=2), min=1.e-4)
                predBGrad = batch_thgpu['finalLocal%sPairwiseSurfaceBPCpredGradOccfloat_hmRenderOm' % bt(edgeOrPlanar)]
                predBNorm = torch.clip(torch.norm(predBGrad, dim=2, p=2), min=1.e-4)
                labelAGrad = batch_thgpu['finalLocal%sPairwiseSurfaceAPCnormalCam_hmRenderOm' % bt(edgeOrPlanar)]
                labelANorm = torch.clip(torch.norm(labelAGrad, dim=2, p=2), min=1.e-4)
                labelBGrad = batch_thgpu['finalLocal%sPairwiseSurfaceBPCnormalCam_hmRenderOm' % bt(edgeOrPlanar)]
    ...
    def forwardBackwardUpdate(self, batch_thgpu, **kwargs):
        ...
        batch_thgpu = self.forwardNetForHm(
            batch_thgpu,
            models=self.models,
            meta=self.meta,
            datasetConf=self.config.datasetConfDict['hmRenderOm'],
                ...
                with torch.no_grad():
                    ...
                    for k in ben.keys():
                        batch_thgpu['statTrain' + bt(k)] = \
                            nanmean(torch.from_numpy(ben[k]).to(self.cudaDeviceForAll))
                    if self.numMpProcess > 0:
                        dist.barrier()
                    self.setModelsTrainingMode(self.models)
