...
class OmnidataDataset(data.Dataset):
    ...
    class Options():
        ...
        '''
        taskonomy_data_path: str = ''
        replica_data_path: str = ''
        gso_data_path: str = ''
        hypersim_data_path: str = ''
        blendedMVS_data_path: str = ''
        split: str = 'train'
    ...
    def load_datasets(self, options):
        ...
        for dataset in self.datasets:
            ...
            ext_raw_root = projRoot + 'external_codes/omnidata_tools/omnidata_tools/torch/'
            if dataset == 'taskonomy':
                tmp_path = ext_raw_root + 'tmp/{}_{}_{}.pkl'.format(
                    dataset,
                    '-'.join(options.tasks), 
            ...
            if tmp_exists and not self.force_refresh_tmp:
                ...
                if dataset == 'taskonomy':
                    dataset_urls = {task: make_taskonomy_dataset(
                        self.taskonomy_data_path, task, self.taskonomy_buildings)
    ...
    def __init__(self, options: Options):
        ...
        for task in self.tasks:
            ...
            for url in self.urls[task]:
                if url.__contains__('replica'):  # ('replica-taskonomized'):
                    building = url.split('/')[-3]
                elif url.__contains__('gso'):  # ('replica-google-objects'): # e.g apartment_0-3, apartment_0-6
                    ...
                    building = url.split('/')[-3]  # [-2]
                elif url.__contains__('blended_mvg'):  # ('BlendedMVS'):
                    building = url.split('/')[-3]
                else:
                    raise NotImplementedError('Dataset path (url) not recognized! %s' %
                                              url)
        ...
        logger.warning("Loaded {} images in {:0.2f} seconds"
                       .format(self.num_images, end_time - start_time))
        logger.warning("\t ({} buildings) ({} points) ({} images) for domains {}"
                       .format(self.num_buildings, self.num_points, self.num_images, self.tasks))
    ...
    def __getitem__(self, index):
        ...
        positive_samples['dataset'] = self.datasets[0]
        positive_samples['view'] = v
        ...
        if max(n_images_task)[0] == min(n_images_task)[0]:
            ...
            def _parse_fpath_for_view( path ):
                ...
                if url.__contains__('replica'):  # ('replica-taskonomized'):
                    ...
                    building = url.split('/')[-3]  # [-2]
                elif url.__contains__('blended_mvs'):  # ('BlendedMVS'):
                    building = url.split('/')[-3]
                ...
                try:
                    ...
                    print('Problem occur! Please check! file_name = %s' % file_name)
                    import ipdb
                    ipdb.set_trace()
                    print(1 + 1)
...
def make_taskonomy_dataset(dir, task, folders=None):
    ...
    for subfolder in folders:
        subfolder_path = os.path.join(dir, subfolder, task)  # )
...
def make_replica_gso_dataset(dir, task, folders=None):
    ...
    for folder in folders:
        if False:  # folder not in REPLICA_BUILDINGS: # gso dataset e.g. apartment_0-3, apartment_0-6, apartment_0-15
        ...
        if not os.path.isdir(folder_path):
            print('Problem occurs! Please check! folder_path = %s' % folder_path)
            import ipdb
            ipdb.set_trace()
            print(1 + 1)
...
def make_hypersim_dataset_orig_split(dir, task, split):
    ...
    folders = sorted([scene for scene in os.listdir(dir) if scene[:10] in set(df['scene_name'].tolist())])
    for folder in folders:
        scene = df.loc[df['scene_name'] == folder[:10]]
        scene_cam = scene.loc[scene['camera_name'] == folder[-6:]]
        for fname in sorted(os.listdir(dir + '/' + folder + '/' + task)):
            point = fname.split('_')[1]
            try:
            ...
            if not row.empty and row.iloc[0]['included_in_public_release'] and row.iloc[0]['split_partition_name'] == split:
                path = os.path.join(dir, folder, task, fname)
                images.append(path)

    # folders = [scene for scene in set(df['scene_name'].tolist()) if scene in [x[:10] for x in os.listdir(dir)]]
    # for folder in folders:
