...
def main():
    ...
    with open('taskonomy_sceneNameList.txt', 'r') as f:
        sceneNameList = f.readlines()
    sceneNameList = [scene_name.strip() for scene_name in sceneNameList]
    with open('taskonomy_subsetNameList.txt', 'r') as f:
        subsetNameList = f.readlines()
    subsetNameList = [subset_name.strip() for subset_name in subsetNameList]
    mylines = []
    for scene_name in sceneNameList:
        for subset_name in subsetNameList[:-1]:  # ignore mask_valid for now.
            mylines.append('https://datasets.epfl.ch/taskonomy/%s_%s.tar' %
                           (scene_name, subset_name))
    missinglines = []
    for i, myline in enumerate(mylines):
        if myline not in lines:
            missinglines.append(myline)
            if '_rgb.' in myline:
                print(myline)
    import ipdb
    ipdb.set_trace()
    print(1 + 1)
    raise NotImplementedError('This script is not intended for running')
    # Please stick to script2
