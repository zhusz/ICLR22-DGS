...
# This file is the modified version of https://github.com/ScanNet/ScanNet/blob/master/SensReader/python/SensorData.py
...
class RGBDFrame():
  ...
  def load(self, file_handle):
    ...
    # color_data = struct.unpack('c'*self.color_size_bytes, file_handle.read(self.color_size_bytes))
    # self.color_data = ''.join((chr(int.from_bytes(x, 'big')) for x in color_data[:20]))
    # depth_data = struct.unpack('c'*self.depth_size_bytes, file_handle.read(self.depth_size_bytes))
    # self.depth_data = ''.join((chr(int.from_bytes(x, 'big')) for x in depth_data))
  ...
  def load(self, filename):
    with open(filename, 'rb') as f:
      ...
      for i in range(num_frames):
        # if i % 100 == 0:
        #   print('Loading %s: %d / %d' % (filename, i, num_frames))
        frame = RGBDFrame()
        frame.load(f)
