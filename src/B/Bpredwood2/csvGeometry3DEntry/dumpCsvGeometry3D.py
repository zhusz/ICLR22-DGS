...
governedScriptTagList = ['1E', '1Epp1', '1G']
...
# Note, when reporting numbers, you cannot have an approach's validCount smaller than other approaches.
# An approach with validCount smaller than other approaches cannot be reported. (inf chamfer, 0 F1 score)
# When all the approaches fail on one test case (e.g. -475), then rule out this sample for all the approaches.
def exitWithNanBenchmarking(bsv0):  # Use for 'ignoreAndNan', not 'ignoreAndNone'
    keys_3D = []
...
def assertBenchmarkings(bsv0_toStore, bsv0_loaded, **kwargs):
    ...
    for k in ['MetricR1', 'FitR1']:  # , 'MetricR1ViaD', 'MetricF1ViaD', 'FitR1ViaD']:
        ...
        if not delta < thre_strict:
            _wrong()
    thre_lenient = 1.e-3  # if not approachNickName.startswith('P16') else 1.e-3
...
def get_s(bsv0_retrieveList_dict, approachEntryDict, testDataEntry, **kwargs):
    ...
    for approachNickName in bsv0_retrieveList_dict.keys():
        ...
        s[approachNickName]['ValidCount'] = np.isfinite(record['MetricF1']).sum()
    return s
