...
import torch
from .dumpCsvGeometry3D import exitWithNanBenchmarking
import numpy as np
import trimesh
...
from codes_py.toolbox_3D.draw_mesh_v1 import pickInitialObservationPoints
...
def benchmarkingGeometry3DScannetFunc(bsv0, **kwargs):
    ...
    assert datasetObj.datasetConf['dataset'].startswith('scannetGivenRender')
    ...
    imgRaw_file_name = datasetObj.cache_dataset_root + datasetObj.fileList_house[houseID] + \
        'color/%d.jpg' % viewID
    img0 = imread(imgRaw_file_name, as_gray=False)
    assert 'uint8' in str(img0.dtype)
    assert img0.max() > 50 and img0.min() >= 0 and img0.max() <= 255
    img0 = img0.astype(np.float32) / 255.
    bsv0['imgRaw'] = img0.transpose((2, 0, 1))
    depthRaw_file_name = datasetObj.cache_dataset_root + datasetObj.fileList_house[houseID] + \
        'depth/%d.png' % viewID
    depthOriginal0 = imread(depthRaw_file_name)
    ...
    assert np.nanmin(depth0) >= 0
    bsv0['depthRaw'] = depth0[None, :, :]
    del houseID, viewID, imgRaw_file_name, img0, depthOriginal0, depthRaw_file_name, depth0, maskUnknown0
    ...
    for k in ['focalLengthWidth', 'focalLengthHeight', 'winWidth', 'winHeight', 'fScaleWidth', 'fScaleHeight']:
        bsv0[k] = datasetConf[k]
    ...
    del tmp0, fullVertWorld0, fullFace0, nFace, faceFlag0, fullFaceRgb0, viewVertWorld0, viewFace0, houseID
    ...
    # bound[Min|Max][Cam|World]
    bsv0['boundMinCam'] = bsv0['viewVertCam'].min(0)
    bsv0['boundMaxCam'] = bsv0['viewVertCam'].max(0)
    bsv0['boundMinWorld'] = bsv0['viewVertWorld'].min(0)
    bsv0['boundMaxWorld'] = bsv0['viewVertWorld'].max(0)
    ...
    # coloring evalPredMetric and evalPredFit, as well as evalViewMetric and evalViewFit
    for metricOrFit in ['fit', 'metric']:
        sysLabel = 'world'
        tmp0 = packageCDF1(
