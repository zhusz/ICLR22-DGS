...
def main():
    ...
    if testDataNickName in ['squareFreedemo1']:
        indVisChosen = [7]  # [2, 7]
    elif testDataNickName in ['squareFreedemo3']:
        indVisChosen = [5]  # [3, 4, 5, 9]
    elif testDataNickName in ['hmFsOfficialValSplit10']:
        indVisChosen = [8570531, 9087701]
    else:
    ...
    ph = int(54 * 1.2)
    # R = len(views) * len(ind)
    # C = 1 + len(methodologyNameList)  # input, methods
    R = len(methodologyNameList) * len(indVisChosen)
    C = 1 + len(views)
    ...
    for i, j in enumerate(indVisChosen):
        ...
        for ic, methodologyName in enumerate(methodologyNameList):
            ...
            for iv, v in enumerate(views):
                # r = i * len(views) + iv
                # c = 1 + ic
                r = ic + i * len(methodologyNameList)
                c = 1 + iv
                cloth[
                ...
                #     :
                # ] = cv2.resize(
                #     bsv0_forVis['evalViewWorldVertRgbMeshDrawingPackageViewColor'][v, :, :, :],
        ...
        #         (w + intervalW) * c:(w + intervalW) * c + w,
        #         :
        #     ] = bsv0_forVis['evalView2WorldVertRgbMeshDrawingPackageViewColor'][v, :, :, :]

    cloth = np.clip(cloth, a_min=0, a_max=1)
    fn = visualDir + studioTag + '_%s' % testDataNickName
    for i in indVisChosen:
        fn += ('_' + '%d' % i)
    fn += '.png'
    imsave(fn, cloth)
