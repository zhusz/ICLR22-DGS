...
from codes_py.toolbox_framework.framework_util_v2 import splitPDSRI, splitPDDrandom, \
    constructInitialBatchStepVis0, mergeFromBatchVis, mergeFromAnotherBsv0
...
def debugCheckDiff(bsv0):
    with open('tmp.pkl', 'rb') as f:
        bb = pickle.load(f)
    cc = []
    for k in bb.keys():
        if type(bb[k]) is np.ndarray:
            if not np.all(bb[k] == bsv0[k]):
                cc.append(k)
        elif type(bb[k]) in [int, float, str]:
            if not bb[k] == bsv0[k]:
                cc.append(k)
    return cc
    ...
    for i, j in enumerate(indChosen):
        ...
        if os.path.exists(outputBenFileName) and \
            ...
            bsv0_initial = constructInitialBatchStepVis0(
                batch_vis, iterCount=int(methodologyI[1:]), visIndex=0,
            ...
            if os.path.exists(outputBenFileName):
                ...
                if testDataNickName in ['scannetOfficialTestSplit10']:
                    assertBenchmarkings(bsv0_toStore, bsv0_loaded, verbose=False)
...
def main():
    ...
    # specify
    if os.environ['MC'] == 'all':
        for mc in approachEntryDict.keys():
            if t1[1] == approachEntryDict[mc]['scriptTag']:
                testApproachOnTestData(testDataEntry, approachEntryDict[mc])
    else:
        approachEntry = approachEntryDict[os.environ['MC']]
        assert t1[1] == approachEntry['scriptTag']
